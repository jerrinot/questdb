...
| 4 | `longSpecies()` called 2x in setup (offsets 6, 37) | Low | Only matters at method entry, not per-chunk. |
...
| 4 | 7x `checkcast` in loop body | Low | C2 eliminates. 2 for longNullGt args, 2 for doubleVecLt args, 2 for mask.and, 1 for mask.and(activeMask). |
...
## Filter 3: `l IN (1, 2, 3, 4, 5)` — vectorized (straight-line OR)

For single-size columns (all LONG), the IR serializer does NOT use
short-circuit opcodes. The `ENABLE_SHORT_CIRCUIT && scalarModeDetected`
guard is false because `scalarModeDetected` requires mixed column sizes
or force-scalar mode. Instead, the serializer emits plain `EQ` + `OR`
ops:

```
IMM 1, MEM col0, EQ,
IMM 2, MEM col0, EQ, OR,
IMM 3, MEM col0, EQ, OR,
IMM 4, MEM col0, EQ, OR,
IMM 5, MEM col0, EQ, OR,
RET
```

This is a straight-line program — `hasControlFlow() == false` — so the
vector compiler accepts it. The generated bytecode loads the column once
per value, broadcasts each constant, compares via `LongVector.compare(EQ)`,
and ORs the mask results together.

For IN() with **mixed-size columns** (e.g., INT and LONG), the serializer
DOES use short-circuit, producing `hasControlFlow() == true`, which falls
back to scalar bytecode.
...
| `l IN (1, 2, 3, 4, 5)` | Yes (straight-line OR) | NATIVE_SIMD uses AVX2 |
| `l > 0 AND i != 0 AND d < 0.5 AND l < 1000000` | **No** (I4 unsupported) | NATIVE_SIMD uses AVX2 |

Note: The fourth filter is NOT vectorized because column `i` (INT) maps
to `I4_TYPE`, which `isSupportedVectorType()` rejects (only I8 and F8
are supported).
...
|----------|-------------|------|-----------------|
| 1 | Support I4 (INT) columns for vectorization | Codegen | High: fourth benchmark filter rejected |
...
| 5 | Reduce column re-loads in IN() (same column loaded N times) | Codegen | Medium: IN(v1..v5) loads the column 5 times instead of once |
